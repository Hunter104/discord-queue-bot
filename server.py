    ...
    async def expiry_timer(self):
        try:
            ...
            if sleep_period <= timedelta(0):
                logger.warning(f"Expiration date {self.current_status.current_user} has already been reached for user {self.current_status.current_user}")
            else:
                logger.info(f"User %s will expire in %s, sleeping for %.2f seconds", self.current_status.current_user, sleep_period, sleep_period.total_seconds())
