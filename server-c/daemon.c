...
void *update_heartbeat_loop(void *arg) {
  ...
  redisContext *c = redisConnect(status.redis_ip, status.redis_port);
  if (c == NULL || c->err) {
    if (c) {
...
void *fetch_user_loop(void *arg) {
  ...
  while (true) {
    if (!status.status.is_occupied) {
      ...
      strncpy(status.status.current_user, next_user, MAX_NAME_SIZE);
      pthread_mutex_unlock(&status_lock);
    ...
    int sleep_duration = status.status.expiry - time(NULL);
    if (sleep_duration < 0) {
    ...
    rc = release_user(c, status.status.current_user);
    CHECK_RESULT(rc);
  }
  return NULL;
}
...
int main(int argc, char *argv[]) {
  ...
  rc = pthread_create(&user_thread, NULL, (void *(*)(void *))fetch_user_loop,
                      NULL);
  if (rc != 0) {
