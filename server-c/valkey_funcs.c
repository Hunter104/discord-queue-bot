...
int register_host(redisContext *c, char *hostname) {
  redisReply *reply = (redisReply *)redisCommand(
      c, "SADD %s %s", REGISTERED_HOSTS_KEY, hostname);
  if (reply == NULL) {
...
int pop_waiting_queue(redisContext *c, int timeout, char *str, size_t n) {
  int rc = 0;
  redisReply *reply = (redisReply *)redisCommand(
      c, "BLMOVE %s %s LEFT RIGHT 0", WAITING_QUEUE_KEY, PROCESSING_QUEUE_KEY);
  ...
  reply = (redisReply *)redisCommand(c, "SET %s:%s:%s %s NX EX %d", USER_PREFIX,
                                     str, USER_LOCK_SUFFIX, "locked", timeout);
  CHECK_REPLY(c, reply, rc);
...
int release_user(redisContext *c, char *username) {
  int rc = 0;
  redisReply *reply = (redisReply *)redisCommand(c, "DEL %s:%s:%s", USER_PREFIX,
                                                 username, USER_LOCK_SUFFIX);
...
redisReply *set_status_command(redisContext *c, HostStatus hostStatus,
  ...
  redisReply *reply = (redisReply *)redisCommand(
      c,
      "HSETEX %s:%s:%s EX %d FIELDS 5 hostname %s is_occupied %d expiry "
      "%lu current_user %s last_timestamp %lu",
      HOST_PREFIX, hostStatus.hostname, HOST_STATUS_SUFFIX, timeout,
      hostStatus.hostname, hostStatus.is_occupied, hostStatus.expiry,
      hostStatus.current_user, hostStatus.last_timestamp);
  return reply;
}
...
int set_status(redisContext *c, HostStatus hostStatus, int timeout) {
  ...
  if (hostStatus.is_occupied) {
    reply = redisCommand(c, "SET %s:%s:%s %s EX %d", USER_PREFIX,
                         hostStatus.current_user, USER_ASSIGNMENT_SUFFIX,
...
int get_status(redisContext *c, char *hostname, HostStatus *hostStatus) {
  int rc = 0;
  redisReply *reply = (redisReply *)redisCommand(
      c, "HGETALL %s:%s:%s", HOST_PREFIX, hostname, HOST_STATUS_SUFFIX);
  CHECK_REPLY(c, reply, rc);
  ...
  hostStatus->is_occupied = atoi(reply->element[3]->str);
...
int finish_assignment(redisContext *c, HostStatus newStatus, int timeout) {
  ...
  // Update user status
  reply = redisCommand(c, "SET %s:%s:%s %s EX %d", USER_PREFIX,
                       newStatus.current_user, USER_ASSIGNMENT_SUFFIX,
                       newStatus.hostname, timeout);
  CHECK_AND_CLEAN_REPLY(c, reply, rc);
  ...
  reply = redisCommand(c, "LREM %s 0 %s", PROCESSING_QUEUE_KEY,
                       newStatus.current_user);
  CHECK_AND_CLEAN_REPLY(c, reply, rc);

  // Publish notification
  reply = redisCommand(c, "PUBLISH %s:%s %s", HOST_PREFIX, newStatus.hostname,
                       newStatus.current_user);
  CHECK_AND_CLEAN_REPLY(c, reply, rc);
